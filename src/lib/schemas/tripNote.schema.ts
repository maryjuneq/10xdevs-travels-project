...
export const BaseTripNoteSchema = z.object({
  ...
  latestStartDate: z.string()
    .regex(/^\d{4}-\d{2}-\d{2}$/, 'latestStartDate must be in ISO-8601 format (YYYY-MM-DD)'),
  
  groupSize: z.number()
    .int('groupSize must be an integer')
    .positive('groupSize must be greater than 0'),
  
  approximateTripLength: z.number()
