...
3. Retrieves the user’s saved preferences (if any).
4. Calls the AI service with note + preferences to obtain an itinerary.
5. Persists the itinerary and logs an `ai_generation_job` record (status _succeeded_ or _failed_).
6. Returns the newly-created itinerary (DTO) on success, or a structured error on failure.
...
3. **API Route** `src/pages/api/trip-notes/generateItenerary.ts` (Astro server endpoint):
   ...
   8. On success:
      ...
      - Log job _succeeded_ with duration.
      - Return 201 with `ItineraryDTO`.
   9. On failure:
      - Log job _failed_ with error text.
