...
Lists all _trip notes_ that belong to the authenticated user.  
...
Returned objects are _light-weight_ representations that include an extra `hasItinerary` flag.
...
- **Query Parameters**
  ...
  | `destination` | string | any<br>(trimmed) | — | Case-insensitive _contains_ filter on destination |
  | `sort` | enum | `destination`\|`earliest_start_date`\|`created_at` + optional `-` prefix | `-created_at` | Sort column (prefix `-` = DESC) |
  | `hasItinerary` | boolean | `true`\|`false` | — | Filter rows that _have / don’t have_ an itinerary |
...
> After implementation, reference the _Trip Notes List_ component in the UI to ensure it aligns with the new response structure.
