...
Server responses are normalised into `FORM_ERROR` union: `INVALID_CREDENTIALS`, `EMAIL_TAKEN`, `TOKEN_EXPIRED`, `UNKNOWN`. All unknown errors fall back to: _"Something went wrong. Please try again."_

### 1.5 Key Scenarios

1. **Happy registration** → user signs up → middleware detects session → redirects `/`.
2. **Login with wrong password** → Supabase returns 400 → form shows generic _"Invalid email or password"_ (no user enumeration).
3. **Password reset** → user enters email → _success_ always displayed to avoid enumeration.
